...
export class AppComponent {
  ...
  cartBooks: Array<bookCarts> = [];

  increaseCart(e: booksCard) {
    let flag:number=0;
    for (let i:number = 0; i < this.cartBooks.length; i++) {
      if (this.cartBooks[i].ISBN === e.ISBN) {flag=1;break;}
    }
    let x:bookCarts={...e,quantity:1};
    if(flag===0) 
  ...
  modifyIncCart(e:number){
    let flag:number=0;
    for (let i:number = 0; i < this.cartBooks.length; i++) {
      if (this.cartBooks[i].ISBN === e) 
      {
        this.cartBooks[i].quantity++;
        this.total=this.total+this.cartBooks[i].price.value;
        console.log(this.cartBooks[i].quantity);break;
      }
    }
  }
  modifyDecCart(e:number){
    let p:bookCarts|undefined
    for (let i:number = 0; i < this.cartBooks.length; i++) {
      if (this.cartBooks[i].ISBN === e) 
      {
        this.cartBooks[i].quantity--;
        ...
        if(this.total<0)
        this.total=0;
        if(this.cartBooks[i].quantity==0)
        p= this.cartBooks.find(x=> x.ISBN===e);
        if(p){
          this.decreaseCart(p);
        }
        console.log(this.cartBooks[i].quantity);break;
